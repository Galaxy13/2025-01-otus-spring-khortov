...
public class CommentCommands {
    ...
    @Command(command = "find for book", alias = "ffb", description = "Find comments for book by book_id")
    ...
    @Command(command = "find", alias = "cbid", description = "Find specific comment by id")
    public String findCommentById(long id) {
        return commentService.findCommentById(id).map(converter::convertToString)
                .orElse("No comment with id: " + id + " found");
    }

    @Command(command = "save", alias = "cins", description = "Add new comment to book by id")
    ...
    @Command(command = "update", alias = "cupd", description = "Update existing comment")
